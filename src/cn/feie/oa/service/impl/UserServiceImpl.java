...
public class UserServiceImpl implements UserService {
	...
	@Override
	public String getParentById(int uid) {
		String name = null;
		//通过id获取用户
//		User u = userDao.getById(uid);
		//通过id获取职位
		List<Post> postByUid = postDao.getPostByUid(uid);
//		Post p = new Post();
		for (Post post : postByUid) {
			//获取父职位id
			Integer parentid = post.getParentid();
			//通过职位id获取用户
			List<User> user = userDao.getUserByPid(parentid);
			for (User user2 : user) {
				//拼接字符串
				name = user2.getUname();
			}
		...
		Post postBypname = postDao.getPostBypname("部门主管");
		List<User> userByPid = userDao.getUserByPid(postBypname.getPid());
		for (User user : userByPid) {
			//拼接字符串
			name = user.getUname();
		}
		return name;
