...
public class UserAction {
	...
	*/
	...
	public String user_userAdd(User user,String poststrings){
		String[] post = null;
		if (poststrings!=null&&poststrings.equals("")) {
			post = poststrings.split(",");
		}
	...
	public String user_addimager(@RequestParam(value="file",required = false)MultipartFile file,@RequestParam("uid")Integer uid,HttpServletResponse response,HttpServletRequest request,Model model){
		//判断提交过来的是否是文件上传表单
...
//					   String a = request.getRealPath("/")+ "style/image";
...
//          String savedDir = request.getSession().getServletContext().getRealPath("fileDir"); //获取服务器指定文件存取路径    
            String savedDir = request.getRealPath("/")+"style/image/";			//获取服务器指定文件存取路径  
            String u = request.getContextPath()+"/style/image/"+savedFileName;
            userService.addImagerName(u,uid);
